...
dfloat(...)
{
  throw std::runtime_error(
    "Missing function implementation: dfloat");
}
...
{
  throw std::runtime_error(
    "Missing function implementation: dot");
}
...
{
  throw std::runtime_error(
    "Missing function implementation: istlen");
}
  ...
  int& lstrt)
{
  ...
  const int maxl = 100;
  if (lmax > maxl) {
    FEM_STOP("wsphhead >>> lmax too big for format");
  }
  const int maxp = 24;
  if (idp2 > maxp) {
...
legndr(
  ...
  arr_ref<float> x,
  arr_ref<float> xp,
  arr_ref<float> xcosec)
{
  FEM_CMN_SVE(legndr);
  x(dimension(star));
  xp(dimension(star));
  xcosec(dimension(star));
  double& fpi = sve.fpi;
  if (is_called_first_time) {
    fpi = 12.56637062e0;
  }
  ...
  double st = fem::double0;
  int mp1 = fem::int0;
  double fct = fem::double0;
  float sfl3 = fem::float0;
  double compar = fem::double0;
  float dsfl3 = fem::float0;
  double small = fem::double0;
  double x1 = fem::double0;
  double x2 = fem::double0;
  double x3 = fem::double0;
  double cot = fem::double0;
  float cosec = fem::float0;
  int k = fem::int0;
  double f1 = fem::double0;
  double f2 = fem::double0;
  double xm = fem::double0;
  dfloat(i) = fem::ffloat(i);
  sum = 0.e0;
  lp1 = l + 1;
  th = theta;
  ct = fem::dcos(th);
  st = fem::dsin(th);
  mp1 = m + 1;
  fct = fem::dsqrt(dfloat(2 * l + 1) / fpi);
  sfl3 = fem::sqrt(fem::ffloat(l * (l + 1)));
  ...
  x(2) = -st * fct / dsfl3;
  xp(1) = -st * fct;
  xp(2) = -.5e0 * ct * fct * dsfl3;
  if (fem::abs(theta) < 1.e-5f) {
    xcosec(2) = xp(2);
  }
  if (fem::abs(theta) >= 1.e-5f) {
    xcosec(2) = x(2) / st;
  ...
  x1 = x3;
  x2 = -x2 / fem::dsqrt(dfloat(l * (l + 1)));
  FEM_DO_SAFE(i, 3, mp1) {
    k = i - 1;
    f1 = fem::dsqrt(dfloat((l + i - 1) * (l - i + 2)));
    f2 = fem::dsqrt(dfloat((l + i - 2) * (l - i + 3)));
    xm = k;
    x3 = -(2.e0 * cot * (xm - 1.e0) * x2 + f2 * x1) / f1;
    sum += 2.e0 * x3 * x3;
    if (sum - compar > small && i != lp1) {
      return;
    }
    x(i) = x3;
    xcosec(i) = x(i) * cosec;
  ...
  char const* argv[])
{
  ...
  //C
  int id1 = fem::nint(2891.f - dep2);
  int id2 = fem::nint(2891.f - dep1);
  //C
  //C --  read raw file
  cmn.io.open(23, rawfl)
    .status("old");
  int lmax = fem::int0;
  read(23, star), lmax;
  write(6, star), "model lmax =", lmax;
  int np = fem::pow2((lmax + 1));
  const int mxlh = 40;
  if (lmax > mxlh) {
    FEM_STOP("STOP >>>> lmax.gt.MXLH");
  }
  write(6, star), "model np =", np;
  const int mxleny = fem::pow2((mxlh + 1));
  arr<float> rawvec(dimension(mxleny), fem::fill0);
  int i = fem::int0;
  {
    read_loop rloop(cmn, 23, "(5e16.8)");
    FEM_DO_SAFE(i, 1, np) {
      rloop, rawvec(i);
    }
  }
  ...
  //C
  const int maxd = 21;
  const int mxmat = (maxd * (maxd + 1) / 2);
  arr_1d<mxmat, double> ata(fem::fill0);
  ...
  arr_1d<maxd, double> d1(fem::fill0);
  arr_1d<maxd, double> d2(fem::fill0);
  arr_1d<maxd, double> d3(fem::fill0);
  arr_1d<maxd, double> d4(fem::fill0);
  arr_1d<maxd, double> d5(fem::fill0);
  arr_1d<maxd, double> d6(fem::fill0);
  arr_1d<maxd, double> d7(fem::fill0);
  arr_1d<maxd, double> d8(fem::fill0);
  arr_1d<maxd, double> eigv(fem::fill0);
  ...
  float t = fem::float0;
  arr_1d<maxd, double> atd(fem::fill0);
  ...
  //C
  arr_1d<maxd, double> evc(fem::fill0);
  int k = fem::int0;
  double sum = fem::double0;
  double damp = fem::double0;
  double f1 = fem::double0;
  double w = fem::double0;
  arr_1d<maxd, double> x(fem::fill0);
  ...
  FEM_DO_SAFE(i, 1, leny) {
    write(6, star), "AA", i, x(i);
  }
  //C
  ind = 1;
  cmn.io.open(24, sphfl)
    .status("unknown");
  int idp1 = 4;
  int idp2 = 24;
  ...
  FEM_DO_SAFE(j, idp1, idp2) {
    ...
    FEM_DO_SAFE(k, 0, lmax) {
      ind1 = ind + 2 * k;
      {
        write_loop wloop(cmn, 24, "(11e12.4)");
