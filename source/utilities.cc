                          ...
                          const std::string &property_name)
      {
        ...
        if (Patterns::Map(Patterns::Anything(),
        ...
        else if (Patterns::List(Patterns::Double(), 1, list_of_keys.size()).match(input_string))
          {
            ...
            const std::vector<double> values = possibly_extend_from_1_to_N(
                                                 dealii::Utilities::string_to_double(dealii::Utilities::split_string_list(input_string)),
                                                 list_of_keys.size(),
                                                 property_name);
            ...
            if (check_structure)
              {
                AssertThrow((*n_values_per_key)[field_index] == n_values,
                            ExcMessage("The key <" + field_names[field_index] + "> in <" + property_name +
                                       "> does not have "
                                       + "the expected number of values. It expects " +
                                       std::to_string((*n_values_per_key)[field_index])
          ...
          if (var_decl.find(fn_split) != std::string::npos && var_decl[var_decl.length() - 1] == fn_end)
            {
              ...
              std::string var_name = var_decl.substr(var_decl.find(fn_split) + 1,
                                                     var_decl.length() - 2 - var_decl.find(fn_split));
...
//the surface of a sphere.
     ...
     *
     * @note If we modify this code to accept URLs, too, then we can use the
     * netCDF library to read from an OPeNDAP server (and skip the libdap
     * dependency). However, that requires linking against netCDF built with
     * OPeNDAP support.
     *
     * @param filename The local netCDF file name
     * @param data_string A value-result parameter that holds the formatted data
     */
      ...
      for (int i = 0; i < rows; i++)
        {
          for (int j = 0; j < datasetVars.size(); j++)
            {
              data_string += to_string(datasetVars[j][i]);
              data_string += " ";
            }
          data_string += "\n";
        }
      ...
      if (Utilities::MPI::this_mpi_process(comm) == 0)
        {
          ...
          // from a provided URL
          else if (filename.find("http://") == 0 || filename.find("https://") == 0 || filename.find("file://") == 0)
            {
...
// copyright (C) 2011, 2014 Tino Kluge (ttk448 at gmail.com)
                                            ...
                                            const unsigned int component) const
    {
      ...
      if ((dynamic_cast<const GeometryModel::Chunk<dim> *>(&this->get_geometry_model()) != nullptr &&
          ...
          this->get_time() - first_data_file_model_time >= 0.0)
        {
          ...
          const Tensor<1, dim - 1> gradients = lookups.find(boundary_indicator)->second->get_gradients(data_position,
                                               component);

          if (!time_dependent)
            return gradients;

          const Tensor<1, dim - 1> old_gradients = old_lookups.find(boundary_indicator)->second->get_gradients(
                                                     data_position, component);
    ...
    template<int dim>
    double
    AsciiDataBoundary<dim>::get_maximum_component_value(const types::boundary_id boundary_indicator,
                                                        const unsigned int component) const
    {
                       ...
                       const unsigned int component) const
    {
      ...
      double old_difference_in_vertical_position =
        vertical_position - lookups[layer_boundary_index]->get_data(horizontal_position, 0);
      ...
      while (difference_in_vertical_position > 0. && layer_boundary_index < number_of_layer_boundaries - 1)
        {
          ...
          difference_in_vertical_position =
            vertical_position - lookups[layer_boundary_index]->get_data(horizontal_position, 0);
        }
      ...
      if (interpolation_scheme == "piecewise constant")
        ...
        {
          if (difference_in_vertical_position > 0 ||
              layer_boundary_index == 0) // if the point is above the first layer or below the last
            {
              return lookups[layer_boundary_index]->get_data(horizontal_position, component);
            }
          else
            {
              const double f = difference_in_vertical_position /
                               (difference_in_vertical_position - old_difference_in_vertical_position);
      ...
      if (p <= -1000)
      ...
      else if (p == -1)
        {
          ...
          Assert(averaged_parameter > 0,
                 ExcMessage("The sum of the weights/values may not be smaller or equal to zero."));
      ...
      else if (p == 2)
        {
          ...
          Assert(sum_of_weights != 0,
                 ExcMessage("The sum of the weights may not be equal to zero, because we need to divide through it."));
          Assert(averaged_parameter / sum_of_weights > 0,
                 ExcMessage("The sum of the weights is smaller or equal to zero."));
      ...
      else
        {
          ...
          Assert(averaged_parameter > 0,
                 ExcMessage(
                   "The sum of the weights times the values to the power p may not be smaller or equal to zero."));
                                          ...
                                          const double p)
    {
      ...
      if (p <= -1000)
      ...
      else if (p == -1)
        {
          ...
          const double sum_of_weights = std::accumulate(weights.begin(), weights.end(), 0.0);
          Assert(sum_of_weights > 0, ExcMessage("The sum of the weights may not be smaller or equal to zero."));
          return std::pow(averaged_parameter_derivative_part_1 / sum_of_weights, -2) *
                 averaged_parameter_derivative_part_2 / sum_of_weights;
        }
      else if (p == 0)
        {
          // Geometric average
          ...
          Assert(sum_of_weights != 0,
                 ExcMessage("The sum of the weights may not be equal to zero, because we need to divide through it."));
          return std::exp(averaged_parameter_derivative_part_1 / sum_of_weights) * averaged_parameter_derivative_part_2 /
                 sum_of_weights;
        }
      else if (p == 1)
        {
      ...
      else
        {
          ...
          Assert(averaged_parameter_derivative_part_1 / sum_of_weights > 0,
                 ExcMessage(
                   "The sum of the weights times the values to the power p may not be smaller or equal to zero."));
          return std::pow(averaged_parameter_derivative_part_1 / sum_of_weights, (1 / p) - 1) *
                 averaged_parameter_derivative_part_2 / sum_of_weights;
...
#define INSTANTIATE(dim)            \
  ...
  expand_dimensional_variable_names<dim> (const std::vector<std::string> &var_declarations);
    ...
    template
    class AsciiDataLookup<1>;

    template
    class AsciiDataLookup<2>;

    template
    class AsciiDataLookup<3>;

    template
    class AsciiDataBase<2>;

    template
    class AsciiDataBase<3>;

    template
    class AsciiDataBoundary<2>;

    template
    class AsciiDataBoundary<3>;

    template
    class AsciiDataLayered<2>;

    template
    class AsciiDataLayered<3>;

    template
    class AsciiDataInitial<2>;

    template
    class AsciiDataInitial<3>;

    template
    class AsciiDataProfile<1>;

    template
    class AsciiDataProfile<2>;

    template
    class AsciiDataProfile<3>;
    ...
    template
    class NaturalCoordinate<2>;

    template
    class NaturalCoordinate<3>;
